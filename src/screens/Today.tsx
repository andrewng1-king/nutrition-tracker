...
export function Today({ date, setDate }: { date: string; setDate: (d: string) => void }) {
  ...
  const { day, runDay, targets, totals, cost } = dayView(data, date)
  const exById = useMemo(() => exerciseMap(data), [data])
  const lifts = day.lifts ?? []
  const workout = summarize(lifts, exById, data.settings.weightKg)
      ...
      {lifts.length > 0 && (
        <section className="card">
          <div className="between" style={{ marginBottom: 10 }}>
            ...
            <span className="dim">sửa ở tab Bài tập</span>
          </div>
          <div className="grid4" style={{ textAlign: 'center' }}>
              ...
              return (
                <div key={entry.id} className="list-item">
                  ...
                  <span className="entry-kcal">
                    {volumeShort(entryVolume(ex, entry, data.settings.weightKg))}
                  </span>
                </div>
              )
            })}
